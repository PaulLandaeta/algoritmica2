// Dado el segmento a y b hallar el gcd de los números encontrados en el rango de a hasta b 
#include <bits/stdc++.h> 
...
void init(int inicio, int final, int nodoActual) { 
    if( inicio == final ) {
        ...
        segmentTree[nodoActual].lcm = a[inicio];
    } else {
        ...
        segmentTree[nodoActual].lcm = gcd(segmentTree[nodoIzquierdo].lcm,segmentTree[nodoDerecho].lcm);
    } 
}
...
node query(int inicio, int final, int nodoActual, int izquierda, int derecha ) {
    ...
    if(derecha <= mid ) {
        ...
        result.lcm = gcd(maxIzquierdo.gcd, maxDerecho.lcm); 
        return result;
    }      
}
