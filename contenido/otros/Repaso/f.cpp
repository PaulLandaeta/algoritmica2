// Dado 2 enteros en base 2 contar todos los números tal que la suma de sus dígitos es par 
...
long long solveDP(string & number, int index, bool tight, int remainder) {
    ...
    if(dp[index][tight][remainder] == -1) {
        ...
        for( int digit = 0 ; digit<= limit ;digit++) { 
            ...
            numberDivisibleK += solveDP(number,index+1,newTight, digit+remainder);
        }
